...
#    starttime = dt.datetime.combine(dt.datetime.today().date(), dt.time(dt.datetime.today().hour, dt.datetime.today().minute+1))
    starttime = dt.datetime.combine(dt.datetime.today().date(), dt.time(dt.datetime.today().hour+1, 0, 0))
    stoptime = starttime +dt.timedelta(hours = 12)
...
#        print("Scene recorded at"+str(dt.datetime.today()))
    ...
    while starttime < dt.datetime.today() < stoptime:
        now = dt.datetime.combine(dt.datetime.today(), dt.time(dt.datetime.today().hour, dt.datetime.today().minute))
        if now in startlist:
            print("Scene recording triggered at " + str(dt.datetime.today()))
            hScene = recordScene(vidDevList)
            nameScene(hScene, vidDevList)
            print("Scene recording complete at "+str(dt.datetime.today()))
