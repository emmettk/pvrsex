...
def exportScene(vidDevList, exportPath, fileType = "TIFF", nReduceFactor = 3):
    """
    ...
    for i in xrange(len(vidDevList)):
        expDevName = sGetName(vidDevList[i])
        nNumImagesRecorded = sGetNumRecordedFrames(0, getActiveScene(), vidDevList[i])
        ...
        sSetDeviceViewerSelection(vidDevList[i], 0, 0, 0)
        for nImageIndex in xrange(nNumImagesRecorded):
            if fmod(nImageIndex, nReduceFactor) == 0:
                imgsaved +=1
                ### Keep current selection
                sSetDeviceViewerSelection(vidDevList[i], nImageIndex, nImageIndex, 1)
        ...
        print(expFName)
        eErr = sExport([vidDevList[i]], hExport, expFName, xmVIEWER_SELECTION, exportFlags)
...
def main(exportPath, nReduceFactor):
    ...
    if addExportDir(exportPath, inputok = False):
        ...
        exportScene(vidDevList, exportPath, fileType = "TIFF", nReduceFactor = nReduceFactor)

if __name__ == "__main__":
    exportPath = "C:\\StreamsExportTest2\\ReducedVideoTest4\\"
    nReduceFactor = 300
    main(exportPath, nReduceFactor)
